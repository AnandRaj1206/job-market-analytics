...
const api = axios.create({
  baseURL: "https://job-market-analytics-e5hu.onrender.com/api",
});

export const getTopSkills = () =>
  api.get("/analytics/top-skills").then((r) => r.data);

export const getSalaryTrends = () =>
  api.get("/analytics/salary-trends").then((r) => r.data);

export const getTopLocations = () =>
  api.get("/analytics/top-locations").then((r) => r.data);
...
export const getReportDownloadUrl = (category) =>
  `https://job-market-analytics-e5hu.onrender.com/api/chat/report/download?category=${encodeURIComponent(
    category
  )}`;
